...
begin
 ...
 if  not  fq1.fields[0].asinteger > 0 then
   begin
       MessageDlg('El Nº de cuenta '+cnta.ToString+' contable no existe. No se puede realizar el asiento.',mtError,[mbOK],0);
       Exit;
   end;
...
//LA FECHA ES UN PARAMETRO DE ENTRADA, PERO NO SE UTILIZA. NO SE PUEDE MODIFICAR LA FECHA DE UN ASIENTO
   ...
   begin
    ...
    // fq1.ParamByName('FCH').FDDataType:=dtDate;
     ...
     else messagedlg('El Nº de cuenta contable no existe. No se puede modificar el asiento.', mtError,[mbOK],0);
...
begin
if PaginaEnPageControl(principal.PageControl2,'Estadísticas Ventas Compras') then Exit;
...
begin
      if PaginaEnPageControl(principal.PageControl2,'Nóminas') then
      begin
...
begin
     if Pos('Duplicate',E.message)<>0 then
       ShowMessage('Error. El Número de factura ya existe.');
     Action:=daAbort;
